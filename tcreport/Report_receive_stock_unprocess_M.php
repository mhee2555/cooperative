...
<th  width="18 %" align="center">หน่วยนับ</th>
  ...
  $Sql_Detail="SELECT
                DATE(stock_unprocess.Date_start) AS date_s,
                TIME(stock_unprocess.Date_start) AS date_Ts,
                Sum(stock_unprocess.item_qty) AS total_qty,
                Sum(stock_unprocess.item_ccqty) AS total_qtycc,
                type_item.type_name,
                item_unit.UnitName
                FROM
                stock_unprocess
                ...
                WHERE
                MONTH(stock_unprocess.Date_start ) ='$sMonth'
                AND YEAR(stock_unprocess.Date_start ) ='$sYear'
                GROUP BY
                DATE(stock_unprocess.Date_start)
                ORDER BY
...
while ($Result_Detail = mysqli_fetch_assoc($meQuery2)) {
  ...
  $html .=   '<td width="18 %" align="right">'.number_format($Result_Detail['total_qtycc'],0).'</td>';
...
$html .= '<tr nobr="true" style="background-color: #CDCDC1;font-size:18px;" >';
  ...
  $html .=   '<td width="18 %" align="right" style="font-weight: bold;">'.number_format($sump,0).'</td>';
