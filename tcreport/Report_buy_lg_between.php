...
<th  width="15 %" align="center">จำนวนสินค้า</th>
<th  width="15 %" align="center">จำนวนเงิน</th>
<th  width="20 %" align="center">พนักงานซื้อ</th>
<th  width="15 %" align="center">สถานะ</th>
  ...
  $Sql_Detail="SELECT
                ...
                GROUP BY buy_longan.DocNo
                ORDER BY DATE(buy_longan.DocDate) ASC
...
while ($Result_Detail = mysqli_fetch_assoc($meQuery2)) {
  ...
  $html .=   '<td width="15 %" align="right">'.number_format($Result_Detail['total_qty'],0).'</td>';
  $html .=   '<td width="15 %" align="right">'.number_format($Result_Detail['Total'],2).'</td>';
  $html .=   '<td width="20 %" align="center">'.$Result_Detail['FName'].'</td>';

if($Result_Detail['IsStatus']==0){
  $IsStatus="ยังไม่ได้บันทึก";
}else if($Result_Detail['IsStatus']==0){
  $IsStatus="บันทึกเรียบร้อย";
}else{
  $IsStatus="ยกเลิกเอกสาร";
}
  $html .=   '<td width="15 %" align="center">'.$IsStatus.'</td>';
  $html .=  '</tr>';
  $count++;
...
$html .= '<tr nobr="true" style="background-color: #CDCDC1;font-size:18px;" >';
  ...
  $html .=   '<td width="15 %" align="right" style="font-weight: bold;">'.number_format($sumqty,0).'</td>';
  $html .=   '<td width="15 %" align="right" style="font-weight: bold;">'.number_format($sump,2).'</td>';
