...
<th  width="25 %" align="center">จำนวนเงิน</th>
  ...
  $Sql_Detail="SELECT MONTH
                      ( buy_longan.DocDate ) AS M_date,
                      SUM( buy_longan_detail.Total ) AS S_total,
                      ( SELECT SUM( buy_longan_detail.kilo ) ) AS total_qty 
                    FROM
                      buy_longan
                      INNER JOIN buy_longan_detail ON buy_longan.DocNo = buy_longan_detail.Buy_DocNo 
                    WHERE
                      YEAR ( buy_longan.DocDate ) = '$Year' 
                    GROUP BY
                      MONTH ( buy_longan.DocDate )
