...
class MYPDF extends TCPDF
{
  ...
  public function Header()
  {
    ...
    if ($this->page == 1)
    {
      ...
      $this->SetFont('thsarabun', 'b', 22);
      $this->Cell(0, 10,"ใบสำคัญขายสินค้าข้าว", 0, 1, 'C');
      $this->Ln(100);
...
$pdf->Ln(5);
        ...
        $query_detail = "SELECT
                        sale_rice_detail.Sale_DocNo,
                        item.item_name,
                        sale_rice_detail.kilo,
                        packge_unit.PackgeName,
                        sale_rice_detail.total,
                        stock_package.DocNo,
                        packge_unit.Priceperunit
                        FROM
                        sale_rice_detail
                        INNER JOIN item ON sale_rice_detail.item_code = item.item_code
                        INNER JOIN packge_unit ON sale_rice_detail.PackgeCode = packge_unit.PackgeCode
                        INNER JOIN stock_package ON sale_rice_detail.stock_code = stock_package.stock_code
                        WHERE
                        sale_rice_detail.Sale_DocNo = '$DocNo'
                    ";
$c=1;
