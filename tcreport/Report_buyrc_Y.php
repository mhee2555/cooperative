...
<th  width="25 %" align="center">จำนวนเงิน</th>
  ...
  $Sql_Detail="SELECT MONTH
                      ( buy_rice.DocDate ) AS M_date,
                      SUM( buy_rice_detail.Total ) AS S_total,
                      ( SELECT SUM( buy_rice_detail.kilo ) ) AS total_qty 
                    FROM
                      buy_rice
                      INNER JOIN buy_rice_detail ON buy_rice.DocNo = buy_rice_detail.Buy_DocNo 
                    WHERE
                      YEAR ( buy_rice.DocDate ) = '$Year' 
                    GROUP BY
                      MONTH ( buy_rice.DocDate )
