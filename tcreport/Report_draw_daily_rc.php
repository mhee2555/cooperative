...
class MYPDF extends TCPDF
{
  ...
  public function Header()
  {
    ...
    if ($this->page == 1)
    {
      ...
      $this->Cell(0, 10,  "รายงานการขอเบิกข้าว", 0, 1, 'C');
...
$pdf->AddPage('P', 'A4');
    ...
    $Sql_itemCode="SELECT
                    draw_rice_detail_sub.item_code,
                    item.item_name
                    FROM
                    draw_rice
                    INNER JOIN draw_rice_detail_sub ON draw_rice.DocNo = draw_rice_detail_sub.draw_DocNo
                    INNER JOIN item ON draw_rice_detail_sub.item_code = item.item_code
                    WHERE
                    DATE( draw_rice.DocDate ) BETWEEN '$sDate' AND '$eDate'
                    GROUP BY
                    draw_rice_detail_sub.item_code
                    ORDER BY
                    draw_rice_detail_sub.item_code ASC
                ";
 $pdf->Ln(20);
    ...
    $pdf->Cell(0, 10,$Result_itemCode['item_name'], 0, 1, 'L');
        ...
        $Sql_Detail="SELECT
                        draw_rice.DocNo,
                        draw_rice.DocDate,
                        Sum(draw_rice_detail_sub.kilo) AS total_qty,
                        draw_rice.IsStatus,
                        draw_rice.Approver_ID,
                        item_unit.UnitName,
                        employee.FName
                        FROM
                        draw_rice
                        ...
                        INNER JOIN employee ON draw_rice.Employee_ID = employee.ID 
                        WHERE
                        DATE( draw_rice.DocDate ) BETWEEN '$sDate' AND '$eDate'
                        AND draw_rice_detail_sub.item_code='$item_code'
                        GROUP BY
                        draw_rice.DocNo
                        ORDER BY
                        draw_rice.DocDate ASC
                    ";
                    $sump=0;
                    $sumqty=0;
                    $count = 1;
