...
<th  width="25 %" align="center">หน่วยนับ</th>
  ...
  $Sql_Detail="SELECT
              ...
              emp.FName AS employee,
              sl.RefDocNo,
              stockp_longan_detail.kilo,
              item.item_name,
              item_unit.UnitName
              FROM
              stockp_longan AS sl
              INNER JOIN employee AS emp ON emp.ID = sl.Employee_ID
              INNER JOIN stockp_longan_detail ON sl.DocNo = stockp_longan_detail.stockp_DocNo
              INNER JOIN item ON stockp_longan_detail.item_code = item.item_code
              INNER JOIN item_unit ON stockp_longan_detail.UnitCode = item_unit.UnitCode
              WHERE
              sl.DocNo = '$DocNo'
                          ";
