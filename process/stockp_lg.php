...
function CreateDocument($conn, $DATA)
{
  ...
  if ($count == 1) 
  {
      $Sqlx = "INSERT INTO stockp_longan (
                    DocNo,
                    DocDate,
                    Modify_Date,
                    Employee_ID
...
function Savebill($conn, $DATA)
{
  ...
  //UPDATE STATUS 
  $Sql = "UPDATE stockp_longan SET IsStatus = 1 , Modify_Date = TIME(NOW())  WHERE stockp_longan.DocNo = '$DocNo'";
...
function SaveRefDocNo($conn, $DATA)
{
  ...
  $DocNo  = $DATA["DocNo"];
  // ===========================================
  $updateRef = "UPDATE stockp_longan , process_longan SET stockp_longan.RefDocNo = '$RefDocNo' , process_longan.IsRef = 1 WHERE stockp_longan.DocNo = '$DocNo' AND process_longan.DocNo = '$RefDocNo' "; 
...
function Importdata($conn, $DATA)
{
  ...
  foreach ($item_codex as $key => $value)
  {
    ...
    if ($chkUpdate == 0) 
    {
      $insert = "INSERT INTO  stockp_longan_detail
                  SET 
                      stockp_DocNo = '$DocNo',
                      item_code = '$value',
                      kilo = '$kilox[$key]' ";
    ...
    {
      $update = "UPDATE  stockp_longan_detail
                 SET 
                      stockp_DocNo = '$DocNo',
                      item_code = '$value',
                      kilo = ( kilo + '$kilox[$key]' )
